...
def runMapReduceWordCount(hosts, args):
    try:
        ...
        # Start the master
        print "Running cmd: python ./mr_wordcount.py -m ", str(args.map), " -r ", str(args.reduce), " -p ", str(
            args.masterport), " ", args.datafile, " on host: ", hosts[0].IP()
        # popens [ master ] = master.popen ('ping', '-c3', master.IP() )
        popens[hosts[0]] = hosts[0].popen('python', './mr_wordcount.py', '-m', str(args.map), '-r', str(args.reduce),
                                          '-p', str(args.masterport), args.datafile, stdout=subprocess.PIPE)
        ...
        for i in range(args.map):
            ...
            print "Running cmd: python ./mr_mapworker.py ", str(i), " ", hosts[0].IP(), " ", str(
                args.masterport), " on host: ", hosts[i + 1].IP()
            popens[hosts[i + 1]] = hosts[i + 1].popen('python', './mr_map.py', str(i), hosts[0].IP(),
                                                      str(args.masterport), stdout=subprocess.PIPE)
